 ...
 *   用户信息
 * @return
func QueryTrainMessage(u login.User, conversation *conversation.Conversation) (tran bean.Train, err error) {
	req, _ := http.NewRequest(http.MethodGet, api.Query+"O?leftTicketDTO.train_date="+u.RideDate+"&leftTicketDTO.from_station="+stations.GetStationValueByKey(u.Departure)+"&leftTicketDTO.to_station="+stations.GetStationValueByKey(u.Terminus)+"&purpose_codes=ADULT", nil)
 ...
 * @return
 */
func FormatQueryMessage(s []string) (trans []bean.Train, err error) {
	defer func() {
 ...
 * @param stationCode
 */
func PrintTrainMessage(trans []bean.Train) {
	fmt.Print("序号\t")
	fmt.Print("车次\t")
	fmt.Print("始发站\t")
	fmt.Print("终到站\t")
	...
	for i := 0; i < len(trans); i++ {
		...
		fmt.Print(trans[i].From + "\t")
		fmt.Print(trans[i].To + "\t")
		fmt.Print(trans[i].FindFrom + "\t")
		fmt.Print(trans[i].FindTo + "\t")
